...
(defn get-group
  ...
  (-> keycloak-client (.realms) (.realm realm-name) (.groups) (.group group-id)))

(defn list-subgroups
  [keycloak-client realm-name group-id]
  (info "List all subgroups of group" group-id "in realm " realm-name)
  (-> (get-group keycloak-client realm-name group-id) (.toRepresentation) (.getSubGroups)))

(defn get-subgroup-id
  [keycloak-client realm-name group-id subgroup-name]
  (-> (filter #(= subgroup-name (.getName %)) (list-subgroups keycloak-client realm-name group-id)) (first) (.getId)))

(defn create-subgroup!
  [keycloak-client realm-name group-id subgroup-name]
  (info "create subgroup" subgroup-name "in group" group-id "in realm" realm-name)
  (-> (get-group keycloak-client realm-name group-id) (.subGroup (group-representation subgroup-name))))
...
(defn create-user!
  ...
  (-> keycloak-client (.realms) (.realm realm-name) (.users) (.create (user-representation username))))

(defn add-user-to-group!
  [keycloak-client realm-name group-id user-id]
  (info "add user" user-id "in group" group-id "of realm" realm-name)
...
(defn add-user-to-subgroup!
  [keycloak-client realm-name subgroup-id user-id]
  (info "add user" user-id "in subgroup" subgroup-id "of realm" realm-name)
